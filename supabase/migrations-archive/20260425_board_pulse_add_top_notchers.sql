-- Create top_notchers table in board_pulse schema
-- Based on: 20260322_add_top_notchers.sql
...
create unique index if not exists top_notchers_exam_rank_key
  on board_pulse.top_notchers (exam_id, rank);

create unique index if not exists top_notchers_exam_full_name_key
  on board_pulse.top_notchers (exam_id, full_name);