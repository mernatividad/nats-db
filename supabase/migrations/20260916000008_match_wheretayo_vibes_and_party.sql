-- Extend matcher scoring with party size and weighted vibe taxonomy.
drop function if exists wheretayo.match_venues(smallint, text, text, uuid, integer);
  ...
  with scored as (
    ...
    left join lateral (
      select coalesce(sum(case
        when p_energy = 'work' and vibe.slug = 'work-friendly' then vv.weight * 3
        when p_energy = 'slow' and vibe.slug = 'slow-afternoons' then vv.weight * 3
        when p_energy = 'night' and vibe.slug = 'late-night-buzz' then vv.weight * 3
        when p_non_negotiable = 'outdoor' and vibe.slug = 'outdoor-tables' then vv.weight * 3
        when p_party in ('pair', 'small') and vibe.slug = 'good-for-groups' then vv.weight * 3
        else 0
      end), 0)::integer as vibe_score
      from wheretayo.venue_vibes vv
      join wheretayo.vibes vibe on vibe.id = vv.vibe_id
      where vv.venue_id = v.id
    ) vibe on true
