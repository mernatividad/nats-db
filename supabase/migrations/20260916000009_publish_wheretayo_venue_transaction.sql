-- Atomically publish a fully reviewed venue with its evidence and vibe weights.
-- This function is intentionally service-role-only and is not callable by the browser.
...
begin
  ...
  if jsonb_typeof(coalesce(p_vibes, '{}'::jsonb)) <> 'object' or jsonb_object_length(coalesce(p_vibes, '{}'::jsonb)) = 0 then
  ...
  select v_venue_id, fact.key, fact.value, 'admin', 'verified'
    from jsonb_each_text(p_facts) fact;

  insert into wheretayo.venue_vibes (venue_id, vibe_id, weight)
  select v_venue_id, v.id, (requested.weight)::smallint
  ...
  insert into wheretayo.venue_attribute_evidence (venue_id, attribute_name, attribute_value, evidence_type, confidence)
  select v_venue_id, 'vibe:' || requested.key, requested.value, 'admin', 'verified'
    from jsonb_each_text(p_vibes) requested;
...
revoke all on function wheretayo.publish_venue(text, text, text, jsonb, jsonb) from public;
grant execute on function wheretayo.publish_venue(text, text, text, jsonb, jsonb) to service_role;
