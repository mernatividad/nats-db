-- Resolver input must identify an official PRC article. Otherwise a page
-- generated by this application can accidentally become a second release.
create or replace function board_pulse.is_official_prc_article_url(p_url text)
returns boolean
language sql
immutable
strict
set search_path = ''
as $$
  select p_url ~* '^https?://(www\.)?prc\.gov\.ph/(article/|[^[:space:]/]+/article/)'
$$;
