...
declare
  ...
  r board_pulse.prc_releases%rowtype;
  effective_date date;
begin
  ...
  update board_pulse.prc_releases as rel set official_release_date = coalesce(rel.official_release_date, effective_date),
    official_release_date_source_url = coalesce(rel.official_release_date_source_url, p_source_url),
    official_release_date_source_hash = coalesce(rel.official_release_date_source_hash, p_source_hash),
    official_release_date_sources = coalesce(rel.official_release_date_sources, p_sources),
    official_release_date_parser_version = coalesce(rel.official_release_date_parser_version, p_parser_version), updated_at = now()
  where rel.id = e.release_id;
  insert into board_pulse.official_release_date_audit(run_id, operator, exam_id, release_id, action, old_date, new_date, source_hash, reason, idempotency_key)
  ...
  if coalesce(p_validation_status, 'validated') = 'validated' then
    ...
    values(e.id, 'exam_results_released', p_idempotency_key, jsonb_build_object(
      'exam_id', e.id, 'release_id', e.release_id, 'exam_name', e.name, 'exam_slug', e.slug,
      'article_url', e.source_article_url, 'source_url', e.source_article_url,
      'passers_pdf_url', e.passers_pdf_url, 'top_notchers_pdf_url', e.top_notchers_pdf_url,
      'passers_path', '/' || e.slug || '-passers', 'top_notchers_path', '/' || e.slug || '-top-notchers',
