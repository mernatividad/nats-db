...
begin
  if position('is_official_prc_article_url' in lower(definition)) = 0 then
    raise exception 'resolve_or_create_prc_exam does not validate official PRC article sources';
  end if;
